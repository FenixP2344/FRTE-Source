...
class HealAction extends SwatCharacterAction;
///////////////////////////////////////////////////////////////////////////////
...
// the cuffs
var private HandheldEquipment			FieldDress;

// arrested sensor
//var private ArrestedSensor				TargetArrestedSensor;

// config
var config float						DistanceFromTargetToHeal;
...
function cleanup()
{
	...
	// if the FieldDress are in the middle fo being used, interrupt it
	if (FieldDress != None)
	{
		if (FieldDress.IsBeingUsed())
		{
			IAmAQualifiedUseEquipment(FieldDress).InstantInterrupt();
		}

		FieldDress.AIInterrupt();
	}
	...
	if (class'Pawn'.static.checkConscious(InjuredTarget))
	{
		InjuredTarget.EnableCollisionAvoidance();
	}
...
latent function MoveIntoPosition()
{
	...
	CurrentMoveToActorGoal = new class'MoveToActorGoal'(movementResource(), achievingGoal.priority, InjuredTarget);
	...
	CurrentMoveToActorGoal.SetMoveToThreshold(InjuredTarget.CollisionRadius + DistanceFromTargetToHeal);
...
latent function RotateTowardsTarget()
{
	...
	RotationToTarget = rotator(InjuredTarget.Location - m_Pawn.Location);
...
latent function HealTarget()
{
	if (IAmUsedOnOther(FieldDress).CanUseOnOtherNow(InjuredTarget))
	{
		FieldDress.LatentWaitForIdleAndEquip();

		// use the FieldDress on the target
		if (IAmUsedOnOther(FieldDress).CanUseOnOtherNow(InjuredTarget))
		{
			ISwatOfficer(m_Pawn).GetOfficerSpeechManagerAction().TriggerReassurePassiveHostageSpeech();
			IAmUsedOnOther(FieldDress).LatentUseOn(InjuredTarget);
		}
...
private function TriggerFinishedSpeech()
{
