...
private function PopulateCampaignMissionList()
{
    ...
    {
		for(index = 0; index < class'SwatGame.SwatClassicCareerPath'.default.Missions.Length; index++)
  		{
  			if( index <= theCampaign.GetAvailableIndex() )
            {
				MyMissionSelectionBox.List.Add(string(class'SwatGame.SwatClassicCareerPath'.default.Missions[index]),,
					class'SwatGame.SwatClassicCareerPath'.default.MissionFriendlyNames[index],
					index,, true);
            }
  	     }
  	}
