class Do_TimedSpecial extends Do_Special;

var config bool bFreezeTimerOnSuccess;
...
function Initialize()
{
    ...
    assertWithDescription(Time > 0,
        "[Do_TimedSpecial] Timed special objective "$name$" has Time=0. Set Time in ObjectiveSpecs to show the timer.");
}

function OnSpecialGameEvent(name SpecialGameEvent)
{

    bFreezeTimerOnNextStop = bFreezeTimerOnSuccess;
